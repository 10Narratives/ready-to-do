...
func (m *Project) validate(all bool) error {
	...
	if !_Project_Name_Pattern.MatchString(m.GetName()) {
		err := ProjectValidationError{
			field:  "Name",
			reason: "value does not match regex pattern \"^projects/[a-z0-9-]{4,30}$\"",
		}
		if !all {
			return err
		}
	...
	if l := utf8.RuneCountInString(m.GetDisplayName()); l < 2 || l > 50 {
		err := ProjectValidationError{
			field:  "DisplayName",
...
var _Project_Name_Pattern = regexp.MustCompile("^projects/[a-z0-9-]{4,30}$")
